...
# [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/ClimateMatchAcademy/course-content/blob/main/tutorials/W1D2_StateoftheClimateOceanandAtmosphereReanalysis/instructor/W1D2_Tutorial2.ipynb)   <a href="https://kaggle.com/kernels/welcome?src=https://raw.githubusercontent.com/ClimateMatchAcademy/course-content/main/tutorials/W1D2_StateoftheClimateOceanandAtmosphereReanalysis/instructor/W1D2_Tutorial2.ipynb" target="_blank"><img alt="Open in Kaggle" src="https://kaggle.com/static/images/open-in-kaggle.svg"/></a>
...
date_sel = datetime.datetime(2018,3,1,0) # select a desired date and hours (midnight is zero)
...
ds_meta = xr.open_dataset(pooch.retrieve(('http://s3.amazonaws.com/'+era5_bucket+"/"+ metadata_key),
                                         known_hash=None )) # open the file
ds_meta
...
selected_vars = ['air_temperature_at_2_metres', 'northward_wind_at_10_metres', 'eastward_wind_at_10_metres',
        'surface_air_pressure','sea_surface_temperature'] # the variables we want
s3_data_ptrn = '{year}/{month}/data/{var}.nc' # path and filename format for this S3 bucket
...
for var in selected_vars: #this will download 5 files of 500Mb each.
    s3_data_key = s3_data_ptrn.format(year=year_s3, month=month_s3, var=var) # variable specific  key
    print("Downloading %s from S3..." % s3_data_key)
    ds_temp = xr.open_dataset(pooch.retrieve(('http://s3.amazonaws.com/'+era5_bucket+"/"+ s3_data_key),
                                             known_hash=None )) #retrieve the variable from the bucket
    if ERA5_select: #check if the dataset is empty or not (first iteration of the loop)
        ERA5_select=xr.merge([ERA5_select,ds_temp]) # if not empty, merge the new file
    else:
        ERA5_select=ds_temp #if empty, just assign the new file

ERA5_allvars = ERA5_select.sel(lon= slice(lon_range[0] , lon_range[1])).sel(lat = slice(lat_range[0] , lat_range[1]))
...
ERA5_allvars['wind_speed'] = np.sqrt(ERA5_allvars['northward_wind_at_10_metres']**2 +
                                    ERA5_allvars['eastward_wind_at_10_metres']**2) # calculate the wind speed from the vectors
# name and units in the DataArray:
ERA5_allvars['wind_speed'].attrs['long_name'] = "10-meter wind speed" # assigning long name to the windspeed
...
dataset_plot = gvDataset(ds_surface_temp_2m_hour.isel(hour=slice(0,12))) # only the first 12 time steps, as it is a time consuming task
images = dataset_plot.to(gvImage, ['longitude', 'latitude'], ['air_temperature_at_2_metres'], 'hour')
images.opts(cmap='coolwarm', colorbar=True, width=600, height=400,projection=ccrs.PlateCarree(),clabel='2m Air Temperature [K]') * gf.coastline
...
dataset_plot = gvDataset(ds_surface_temp_2m_day.isel(day=slice(0,10))) # only the first 10 time steps, as it is a time consuming task
images = dataset_plot.to(gvImage, ['longitude', 'latitude'], ['air_temperature_at_2_metres'], 'day')
images.opts(cmap='coolwarm', colorbar=True, width=600, height=400,projection=ccrs.PlateCarree(),clabel='Air Temperature [K]') * gf.coastline
...
plot_vars = ['air_temperature_at_2_metres', 'wind_speed',
...
for var, ax in  zip(plot_vars, ax_list ):
    ...
    ax.set_ylabel(ERA5_allvars[var].attrs['long_name']+': '+ERA5_allvars[var].attrs['units'],
                  fontsize = 12)
    plt.xticks(rotation = 45)
plt.show()
