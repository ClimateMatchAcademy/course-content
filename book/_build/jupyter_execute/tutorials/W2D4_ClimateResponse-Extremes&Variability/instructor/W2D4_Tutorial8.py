...
wetbulb_hist = xr.open_dataset(pooch.retrieve(url_WBGT_day, known_hash=None)).WBGT
...
wetbulb_126  = xr.open_dataset(pooch.retrieve(url_SSP126, known_hash=None)).WBGT
...
wetbulb_245 = xr.open_dataset(pooch.retrieve(url_SSP245, known_hash=None)).WBGT
...
wetbulb_585 = xr.open_dataset(pooch.retrieve(url_SSP585, known_hash=None)).WBGT
...
fig, ax = plt.subplots()
wetbulb_hist_delhi.groupby('time.year').mean().plot.line(alpha=0.5,c='k',label='hist',ax=ax)
wetbulb_126_delhi.groupby('time.year').mean().plot.line(alpha=0.5,c='C0',label='ssp126',ax=ax)
wetbulb_245_delhi.groupby('time.year').mean().plot.line(alpha=0.5,c='C1',label='ssp245',ax=ax)
wetbulb_585_delhi.groupby('time.year').mean().plot.line(alpha=0.5,c='C2',label='ssp585',ax=ax)
ax.set_title('')
ax.legend()
ax.hlines(
...
plt.plot(wetbulb_126_delhi.time,estimate_return_level_model(1-1/100,law_ns_126),'C0--',label='100-year return level: ssp126')
plt.plot(wetbulb_245_delhi.time,estimate_return_level_model(1-1/100,law_ns_245),'C1--',label='100-year return level: ssp245')
plt.plot(wetbulb_585_delhi.time,estimate_return_level_model(1-1/100,law_ns_585),'C2--',label='100-year return level: ssp585')
...
fit_sp_hist = xr.open_dataset(pooch.retrieve(url_wbgt_hist, known_hash=None,path='~/',fname='wbgt_hist_raw_runmean7_gev.nc'))

# SSP-126 - wbgt_126_raw_runmean7_gev_2071-2100.nc
url_wbgt_126 = "https://osf.io/ef9pv/download"
fit_sp_126 = xr.open_dataset(pooch.retrieve(url_wbgt_126, known_hash=None,path='~/',fname='wbgt_126_raw_runmean7_gev_2071-2100.nc'))

# SSP-245 - wbgt_245_raw_runmean7_gev_2071-2100.nc
url_wbgt_245 = "https://osf.io/j4hfc/download"
fit_sp_245 = xr.open_dataset(pooch.retrieve(url_wbgt_245, known_hash=None,path='~/',fname='wbgt_245_raw_runmean7_gev_2071-2100.nc'))

# SSP-585 - wbgt_585_raw_runmean7_gev_2071-2100.nc
url_bgt_585 = "https://osf.io/y6edw/download"
fit_sp_585 = xr.open_dataset(pooch.retrieve(url_bgt_585, known_hash=None,path='~/',fname='wbgt_585_raw_runmean7_gev_2071-2100.nc'))
...
area = xr.open_dataarray(pooch.retrieve(url_area_mpi, known_hash=None))

# filename - area_land_mpi.nc
url_area_land_mpi = "https://osf.io/dxq98/download"
area_land = xr.open_dataarray(pooch.retrieve(url_area_land_mpi, known_hash=None)).fillna(0.)
...
((wetbulb_126>31)*1).weighted(area_land).mean(['lon','lat']).plot.line('.-',label='ssp126')
((wetbulb_245>31)*1).weighted(area_land).mean(['lon','lat']).plot.line('.-',label='ssp245')
((wetbulb_585>31)*1).weighted(area_land).mean(['lon','lat']).plot.line('.-',label='ssp585')
